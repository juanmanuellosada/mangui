...
Cuentas del usuario: ${input.accounts.join(" | ") || "(ninguna)"}
...
Devolvé SOLO los campos del esquema.`
  ...
  if (input.image) {
    ...
    const prompt = `Extraé los datos de un movimiento financiero a partir de la IMAGEN adjunta (ticket/comprobante/factura). Leé el monto TOTAL, la fecha y el comercio (es-AR). Si el ticket muestra el medio de pago (VISA, MASTERCARD, DÉBITO, EFECTIVO o los últimos dígitos de una tarjeta), usalo para elegir la "cuenta" de la lista que mejor coincida; si no se ve el medio de pago, dejá "cuenta" en null.\n\n${rules}`
