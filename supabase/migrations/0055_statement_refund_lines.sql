-- Soporte para devoluciones/reintegros de impuestos en el import de resúmenes
-- de tarjeta: una línea con is_refund=true (monto siempre positivo, ver
-- src/lib/ai/statement-schema.ts y extract-statement.ts) debe cargarse como
-- ingreso (type='income'), no como gasto, para que el total del ciclo
-- (gastos - reintegros, ver src/lib/cards.ts) cuadre con el TOTAL A PAGAR
-- real del resumen.
--
-- Único cambio respecto de 0053_fix_installment_onconflict.sql: en el INSERT
-- del loop de `lines`, el `type` se decide por `is_refund` en vez del literal
-- 'expense' fijo, y `category_id` se fuerza a NULL para esas líneas (no tiene
-- sentido forzar una categoría de gasto sobre un movimiento de ingreso). El
-- resto de la función (cuotas, recurrentes, idempotencia por
-- import_statement_id, SECURITY DEFINER, grants, ON CONFLICT con WHERE de
-- 0053) queda igual.
