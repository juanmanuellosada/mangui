-- Modo aditivo de import_card_statement para "Corroborar con IA" (Grupo 2 del
-- change corroborar-resumen-con-ia): al aplicar solo las líneas que el
-- usuario tildó del diff (un subconjunto de lo que falta), NO hay que borrar
-- los movimientos simples ya cargados de ese resumen — el import normal sí
-- lo hace (reemplaza el resumen completo), pero corroborar agrega SOBRE lo
-- existente.
--
-- Único cambio respecto de 0055_statement_refund_lines.sql: si
-- p_payload->>'additive' = 'true', se saltea el DELETE previo de movimientos
-- simples (import_statement_id + installment_purchase_id IS NULL). El resto
-- de la función queda igual:
--   - additive ausente/false: comportamiento del import intacto (reemplaza).
--   - additive true: los `lines` del payload se INSERTan como alta nueva,
--     sin tocar los movimientos simples ya cargados.
--   - las cuotas ya eran upsert por (purchase_key/installment_purchase_id,
--     installment_number) — no borran nada, así que idempotencia y
--     no-duplicación quedan iguales en ambos modos.
--   - las recurrentes ya eran idempotentes por source_key — igual en ambos
--     modos.
--   - el upsert de card_statements (totales) queda igual en ambos modos.
