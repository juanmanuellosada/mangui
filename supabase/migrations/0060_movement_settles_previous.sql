-- Devoluciones que cancelan el saldo del resumen ANTERIOR (`settles_previous`).
--
-- Galicia liquida el resumen anterior en un bloque arriba del detalle: saldo
-- anterior − pagos − un crédito de impuestos (ej. "DEV.IMP. RG 5617") = 0. Ese
-- crédito cae dentro del ciclo por fecha, pero NO integra el TOTAL A PAGAR de
-- este resumen: pertenece al anterior.
--
-- Hay que importarlo igual (el cargo que el banco devuelve al mes siguiente
-- quedaría como gasto fantasma en la tarjeta), pero no puede restar del total
-- del ciclo. Hasta ahora la bandera vivía sólo en la capa de extracción, así
-- que `listCardCycles` (cards.ts) —que deriva el total de los movimientos con
-- signedAmount y no tiene el PDF a mano— lo netaba igual: el VISA con cierre
-- 30-ago-26 mostraba $341.870,32 en vez de los $431.289,23 del resumen.
--
-- Se persiste en el movimiento para que la misma regla valga en los tres
-- lugares (extracción, conciliación y total del ciclo). Aditivo: default false
-- deja todo lo ya cargado exactamente como está, y sin la clave en el payload
-- la función se comporta igual que en 0059.

ALTER TABLE public.movements
  ADD COLUMN IF NOT EXISTS settles_previous boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN public.movements.settles_previous IS
  'true si el movimiento es una devolución que cancela el saldo del resumen ANTERIOR de la tarjeta: se cuenta en el saldo de la cuenta pero NO en el total del ciclo (ver listCardCycles en src/lib/cards.ts y ParsedStatementLine.settles_previous en src/lib/ai/statement-schema.ts).';
...
BEGIN
  ...
  LOOP
    INSERT INTO public.movements (
      ...
      v_statement_id,
      COALESCE((v_line->>'settles_previous')::boolean, false)
    );
